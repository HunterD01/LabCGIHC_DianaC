...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Dibujo de la base de prisma cuadrangular
		model = modelaux2;
		...
		*/
		...
		modelaux = model;
		modelaux3 = model;
		model = glm::scale(model, glm::vec3(6.5f, 3.5f, 3.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		//la línea de proyección solo se manda una vez a menos que en tiempo de ejecución
		//se programe cambio entre proyección ortogonal y perspectiva
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		...
		//articulación 1 oreja 1
		model = modelaux2;
		model = glm::translate(model, glm::vec3(-0.5f, 2.4f, -1.0f));
		...
		//oreja 1
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.5));
		//model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		//modelaux = model;
		...
		color = glm::vec3(1.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[1]->RenderMesh(); //dibuja cubo, pirámide triangular, pirámide base cuadrangular
		//meshList[2]->RenderMeshGeometry(); //dibuja las figuras geométricas cilindro y cono

		//oreja 1 contorno rosa
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.5));
		//model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.7f, 1.7f, 2.7f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		//la línea de proyección solo se manda una vez a menos que en tiempo de ejecución
		//se programe cambio entre proyección ortogonal y perspectiva
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		color = glm::vec3(0.956f, 0.796f, 0.890f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[1]->RenderMesh(); //dibuja cubo, pirámide triangular, pirámide base cuadrangular
		//meshList[2]->RenderMeshGeometry(); //dibuja las figuras geométricas cilindro y cono
		
		//articulación 2 oreja 2
		model = modelaux2;
		model = glm::translate(model, glm::vec3(-0.5f, 2.4f, 1.0f));
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		//rotación alrededor de la articulación que une con la cabina
		...
		///patas
		/////////pata 1
		//articulación 3 pata 1 parte1
		model = modelaux3;
		model = glm::translate(model, glm::vec3(-3.25f, -1.75f, 1.75f));
		//rotación alrededor de la articulación que une con la cabina
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion3()), glm::vec3(0.0f, 0.0f, 1.0f));
		modelaux = model;
		model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f));
